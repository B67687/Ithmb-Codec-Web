...
// ---- 1. Key parity ----
const en = JSON.parse(readFileSync(join(LOCALES, "en.json"), "utf8")) as Record<
  string,
  string
>;
const zh = JSON.parse(readFileSync(join(LOCALES, "zh.json"), "utf8")) as Record<
  string,
  string
>;
...
for (const k of enKeys)
  if (!zhKeys.has(k)) fail(`key "${k}" in en.json but NOT in zh.json`);
for (const k of zhKeys)
  if (!enKeys.has(k)) fail(`key "${k}" in zh.json but NOT in en.json`);
...
if (!embMatch) {
  ...
  try {
    ...
    const literal = embMatch[0]
      .slice(embMatch[0].indexOf("{"))
      .replace(/;$/, "");
    embedded = JSON.parse(literal) as Record<string, string>;
  } catch (e) {
    fail(
      "could not parse EMBEDDED_EN: " +
  ...
  if (embedded) {
    ...
    for (const k of embKeys) {
      if (embedded[k] !== en[k]) {
        fail(
          `EMBEDDED_EN value differs from en.json for "${k}" (fallback would show stale copy)`,
        );
      }
    }
  }
