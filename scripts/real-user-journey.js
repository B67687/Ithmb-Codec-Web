const { chromium, firefox } = require("/home/nami/projects/dev/Ithmb-Codec-Web/node_modules/playwright");
...
async function run(browserType, label) {
  ...
  await page.goto("http://localhost:8899/ithmb-decoder/?t=" + Date.now(), { waitUntil: "domcontentloaded" });
  ...
  await chooser.setFiles(["/home/nami/projects/dev/Ithmb-Codec-Web/tests/fixtures/test1.ithmb"]);
  ...
  check("file decodes successfully", status?.includes("Decoded") || status?.includes("解码成功"), status);
  ...
  check("save button updates to PNG format", saveText?.includes("PNG"), saveText);
  ...
  check("footer flips to zh", zhFooter?.includes("奶茶"), zhFooter);

  // 6. Corrupt file -> failure card with share options
  const fs = require("fs");
  const src = fs.readFileSync("/home/nami/projects/dev/Ithmb-Codec-Web/tests/fixtures/test1.ithmb");
  fs.writeFileSync("/home/nami/projects/dev/Ithmb-Codec-Web/tests/fixtures/corrupt-share.ithmb", src.subarray(0, 100));
  const [chooser2] = await Promise.all([page.waitForEvent("filechooser", { timeout: 15000 }), page.locator("#dropzone").click()]);
  await chooser2.setFiles(["/home/nami/projects/dev/Ithmb-Codec-Web/tests/fixtures/corrupt-share.ithmb"]);
  ...
  await page.waitForTimeout(400);
  const formOpen = await page.evaluate(() => !document.querySelector(".file-card .report-form")?.hidden);
  ...
  await page.locator("#langToggle").click();
  await page.waitForTimeout(1200);
  const formStillOpen = await page.evaluate(() => !document.querySelector(".file-card .report-form")?.hidden);
  check("form stays open on lang switch", formStillOpen);

  // 8. Mobile viewport
  const mobile = await ctx.newPage();
  await mobile.setViewportSize({ width: 390, height: 844 });
  await mobile.goto("http://localhost:8899/ithmb-decoder/?t=" + Date.now(), { waitUntil: "domcontentloaded" });
  ...
  await chooser3.setFiles(["/home/nami/projects/dev/Ithmb-Codec-Web/tests/fixtures/test1.ithmb"]);
  await mobile.waitForTimeout(3000);
  check("mobile: decode works", (await mobile.locator(".file-card").count()) === 1);

  // 9. Home page
  await page.goto("http://localhost:8899/?t=" + Date.now(), { waitUntil: "domcontentloaded" });
  await page.waitForTimeout(2000);
  ...
  // 10. 404 page
  await page.goto("http://localhost:8899/nonexistent-page-xyz?t=" + Date.now(), { waitUntil: "domcontentloaded" });
  await page.waitForTimeout(2000);
