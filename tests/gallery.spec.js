...
test.describe("Regression: Viewer pixel content", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(async () => {
      ...
      const hasCanvas = await page.evaluate(
        () => document.querySelector("#viewer-stage canvas") !== null,
      );
  ...
  test("viewer stage canvas has non-blank pixel content", async ({ page }) => {
    ...
    const hasPixels = await page.evaluate(() => {
      const canvas = document.querySelector("#viewer-stage canvas");
      ...
      for (const sheet of sheets) {
        try {
          for (const rule of sheet.cssRules || sheet.rules) {
            if (rule.selectorText?.includes("viewer-placeholder")) return true;
          }
        } catch (e) {}
      }
      return false;
